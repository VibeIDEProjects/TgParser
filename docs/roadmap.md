...
<!-- vibeVersion: 1.0.0 -->
<!-- Последнее обновление: 2026-06-08 (Фаза 7 завершена) -->
...
## Фаза 7: GUI-интерфейс

- [x] Выбор стека технологий: TUI (Textual) vs GUI (PyQt6 / Tkinter) — выбран Textual (TUI)
- [x] Создание прототипа / макета интерфейса
- [x] Реализация главного окна (список каналов, быстрые действия) — `gui/screens/main_screen.py`
- [x] Реализация окна авторизации (ввод API_ID, API_HASH, номера телефона, отображение QR-кода) — `gui/screens/auth_screen.py`
- [x] Реализация окна парсинга:
    - [x] Поле ввода ссылки / @username канала
    - [x] Выбор типа канала (открытый / закрытый)
    - [x] Настройка опций (лимит, фильтр по дате)
    - [x] Кнопка «Старт» с индикатором прогресса
    - [x] Отображение лога парсинга в реальном времени
    - всё в `gui/screens/parse_screen.py`
- [x] Реализация окна результатов / экспорта:
    - [x] Просмотр извлечённых сообщений (текст, дата, автор, медиа)
    - [x] Выбор формата: JSON, CSV, TXT, SQLite
    - [x] Выбор пути сохранения
    - [x] Инкрементальный режим
