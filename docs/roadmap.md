...
- [ ] Выбор стека технологий: TUI (Textual) vs GUI (PyQt6 / Tkinter)
- [ ] Создание прототипа / макета интерфейса
- [ ] Реализация главного окна (список каналов, быстрые действия)
- [ ] Реализация окна авторизации (ввод API_ID, API_HASH, номера телефона, отображение QR-кода)
- [ ] Реализация окна парсинга:
    - [ ] Поле ввода ссылки / @username канала
    - [ ] Выбор типа канала (открытый / закрытый)
    - [ ] Настройка опций (лимит, фильтр по дате)
    - [ ] Кнопка «Старт» с индикатором прогресса
    - [ ] Отображение лога парсинга в реальном времени
- [ ] Реализация окна результатов / экспорта:
    - [ ] Просмотр извлечённых сообщений (текст, дата, автор, медиа)
    - [ ] Выбор формата: JSON, CSV, TXT, SQLite
    - [ ] Выбор пути сохранения
    - [ ] Инкрементальный режим
- [ ] Интеграция с существующими модулями (auth, parsers, storage)
- [ ] Обработка и отображение ошибок в GUI (уведомления, лог)
