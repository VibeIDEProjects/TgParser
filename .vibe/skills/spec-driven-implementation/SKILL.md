...
- `specs/PROJ-1234/PRODUCT.md`
- `specs/PROJ-1234/TECH.md`
...
If the project uses an external issue tracker (Linear, Jira, GitHub Issues, etc.) and a relevant issue does not already exist, create one before writing specs. If the correct team, project, or labels are not obvious from the request and surrounding context, ask the user to clarify rather than guessing.
...
If the feature has UI or interaction design, ask for a design mock if one exists. If there is no mock, continue but call that out explicitly in the product spec.

Reference the tracker issue in the spec when one exists. Because specs live under `specs/<ticket-id>/...`, this should usually be straightforward.
...
- unit tests and regression coverage for critical logic
- integration tests for important user flows
- walkthroughs or feature demos when appropriate
