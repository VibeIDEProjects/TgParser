...
Detailed authoring guidance for creating effective skills in `.claude/skills/`.
...
1. **Be specific and include key terms**
   - Good: "Add a new feature flag to gate code changes."
   - Avoid: "Helps with features."

2. **Include both what and when**
   - What the skill does: "Write, improve, and run unit tests"
...
### Before:
\`\`\`python
if feature_flag.is_enabled("your_feature"):
    # new behavior
else:
