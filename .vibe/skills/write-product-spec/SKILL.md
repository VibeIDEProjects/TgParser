...
"User" is not limited to the end user of the application. It means whoever consumes the surface being designed:
...
- a tracker ticket number (e.g. `specs/PROJ-1234/PRODUCT.md`)
...
Ticket / issue references are optional. If the user has a tracker ticket or GitHub issue, use its id. If they don't, ask them for a feature name to use as the directory. Only create a new ticket or issue when the user explicitly asks for one; ask the user to clarify team, labels, or repo if those are unclear.

## Before writing

Gather only the context you need: directory id (tracker ticket, GitHub issue, or feature name), feature summary, target users, key behaviors, edge cases, and how the feature will be validated. Ask the user for missing context rather than guessing.

### Design mocks

If the feature has any UI or interaction design, ask the user whether a design mock (Figma, Sketch, screenshot, etc.) exists before drafting the Behavior section, and include the link in the spec when one is provided. A mock is often the most reliable source of truth for visual states, spacing, and edge-case layouts — not asking can cause the Behavior section to guess at intent the designer already settled.

- If the user provides a link, include it under a short `## Design` section (or inline near the top of Behavior) as `Design: <link>`.
- If the user confirms no mock exists, note `Design: none provided` so the absence is explicit rather than ambiguous.
...
- **Design** — Include with a link when one exists, or an explicit `Design: none provided` note when design matters but no mock exists. Omit entirely for non-visual features. See "Design mocks" above.
...
A sample Behavior section for a hypothetical feature: rendering GitHub-flavored Markdown tables in a chat or terminal-style block list. It demonstrates the expected shape — numbered, testable, user-perspective invariants that enumerate defaults, edge cases, malformed input, streaming, selection/copy, search, sharing, theming, and cross-surface consistency, with one inline open question.
...
14. Sharing or exporting a block preserves the original markdown source, not the rendered form.

15. Theming: table borders, header backgrounds, alternating row shading (if any), and link/code styles all come from the active app theme. No hard-coded colors.

16. Markdown tables render consistently wherever block-list markdown already renders. The same input produces the same table in each surface.
